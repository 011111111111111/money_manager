...
async function initializeDatabase() {
  ...
  const dbPath = process.env.VERCEL ? '/tmp/expenses.db' : path.join(__dirname, 'expenses.db');
...
app.get('/api/shared-events/:shareCode', async (req, res) => {
  try {
...
app.post('/api/shared-events/:shareCode/expenses', async (req, res) => {
  try {
    const { shareCode } = req.params;
...
app.get('/api/shared-events/:shareCode/expenses', async (req, res) => {
  try {
    const { shareCode } = req.params;
